...
object TemplateGenerator {
  ...
  def apply(modules: Seq[Module]): Seq[Template] = {
    ...
    val enums = modules.collect {
      case enum: EnumType => Template(enum.namespace.replace(".", File.separator) + File.separator + enum.name + ".java", renderer(enum))
    }
    ...
    val records = modules.collect {
      ...
      val defin = records.map(renderer.apply).mkString("\n\n")
      Template(namespace.replace(".", File.separator) + File.separator + "domain.scala", defin)
    }
