...
class JavaEnumEncoderTest extends WordSpec with Matchers {
  ...
  "Encoder" should {
    ...
    "support optional java enums" in {
      ...
      val schema = AvroSchema[Test]
      Encoder[Test].encode(Test(Some(Wine.Malbec)), schema) shouldBe ImmutableRecord(schema, Vector(new EnumSymbol(schema.getField("wine").schema(), "Malbec")))
    ...
    "support scala enums" in {
      ...
      val schema = AvroSchema[Test]
      Encoder[Test].encode(Test(Colours.Amber), schema) shouldBe ImmutableRecord(schema, Vector(new EnumSymbol(schema.getField("value").schema(), "Amber")))
    }
    "support optional scala enums" in {
      case class Test(value: Option[Colours.Value])
      val schema = AvroSchema[Test]
      Encoder[Test].encode(Test(Some(Colours.Green)), schema) shouldBe ImmutableRecord(schema, Vector(new EnumSymbol(schema.getField("value").schema(), "Green")))
    }
  }
}
