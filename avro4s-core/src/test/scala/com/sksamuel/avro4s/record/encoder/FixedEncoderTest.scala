...
class FixedEncoderTest extends FunSuite with Matchers {
  ...
  test("encode fixed when used on a value type") {
    ...
    val record = Encoder[AvroMessage].encode(m, schema, DefaultNamingStrategy).asInstanceOf[GenericRecord]
  ...
  test("encode fixed when used on a field in a case class") {
    ...
    val record = Encoder[FixedString].encode(FixedString("sam"), schema, DefaultNamingStrategy).asInstanceOf[GenericRecord]
  ...
  test("support options of fixed") {
    ...
    val record = Encoder[OptionFixedWrapper].encode(OptionFixedWrapper(Some(FixedValueType("sam"))), schema, DefaultNamingStrategy).asInstanceOf[GenericRecord]
