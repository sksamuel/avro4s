...
import com.sksamuel.avro4s.{AvroSchemaV2, DecoderV2, SchemaForV2}
...
class DateDecoderTest extends AnyFunSuite with Matchers {
  ...
  test("decode int to LocalTime") {
    ...
    DecoderV2[WithLocalTime].decode(record) shouldBe WithLocalTime(LocalTime.of(12, 50, 45))
  }
  ...
  test("decode int to LocalDate") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 17784)
    DecoderV2[WithLocalDate].decode(record) shouldBe WithLocalDate(LocalDate.of(2018, 9, 10))
  }
  ...
  test("decode int to java.sql.Date") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 17784)
    DecoderV2[WithDate].decode(record) shouldBe WithDate(Date.valueOf(LocalDate.of(2018, 9, 10)))
  }
  ...
  test("decode timestamp-millis to LocalDateTime") {
    ...
    DecoderV2[WithLocalDateTime].withSchema(SchemaForV2(schema)).decode(record) shouldBe WithLocalDateTime(
  ...
  test("decode timestamp-nanos to LocalDateTime") {
    ...
    DecoderV2[WithLocalDateTime].decode(record) shouldBe WithLocalDateTime(
  ...
  test("decode long to Timestamp") {
    ...
    DecoderV2[WithTimestamp].decode(record) shouldBe WithTimestamp(new Timestamp(1538312231000L))
  }
  ...
  test("decode long to Instant") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 1538312231000L)
    DecoderV2[WithInstant].decode(record) shouldBe WithInstant(Instant.ofEpochMilli(1538312231000L))
  }
}
