...
class DateDecoderTest extends FunSuite with Matchers {
  ...
  test("decode int to LocalTime") {
    ...
    Decoder[WithLocalTime].decode(record, schema, DefaultNamingStrategy) shouldBe WithLocalTime(LocalTime.of(12, 50, 45))
  }
  ...
  test("decode int to LocalDate") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 17784)
    Decoder[WithLocalDate].decode(record, schema, DefaultNamingStrategy) shouldBe WithLocalDate(LocalDate.of(2018, 9, 10))
  }
  ...
  test("decode int to java.sql.Date") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 17784)
    Decoder[WithDate].decode(record, schema, DefaultNamingStrategy) shouldBe WithDate(Date.valueOf(LocalDate.of(2018, 9, 10)))
  }
  ...
  test("decode long to LocalDateTime") {
    ...
    Decoder[WithLocalDateTime].decode(record, schema, DefaultNamingStrategy) shouldBe WithLocalDateTime(LocalDateTime.of(2018, 9, 10, 11, 58, 59))
  }
  ...
  test("decode long to Timestamp") {
    ...
    Decoder[WithTimestamp].decode(record, schema, DefaultNamingStrategy) shouldBe WithTimestamp(new Timestamp(1538312231000L))
  }
  ...
  test("decode long to Instant") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 1538312231000L)
    Decoder[WithInstant].decode(record, schema, DefaultNamingStrategy) shouldBe WithInstant(Instant.ofEpochMilli(1538312231000L))
  }
}
