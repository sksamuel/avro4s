...
class DateDecoderTest extends AnyFunSuite with Matchers {
  ...
  test("decode int to LocalDate") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 17784)
    Decoder[WithLocalDate].decode(schema).apply(record) shouldBe WithLocalDate(LocalDate.of(2018, 9, 10))
  }
  ...
  test("decode int to java.sql.Date") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 17784)
    Decoder[WithDate].decode(schema).apply(record) shouldBe WithDate(Date.valueOf(LocalDate.of(2018, 9, 10)))
  }
  ...
  test("decode timestamp-millis to LocalDateTime") {
    ...
    Decoder[WithLocalDateTime].decode(schema).apply(record) shouldBe WithLocalDateTime(
  ...
  test("decode long to Timestamp") {
    ...
    Decoder[WithTimestamp].decode(schema).apply(record) shouldBe WithTimestamp(new Timestamp(1538312231000L))
  }
  ...
  test("decode timestamp-micros to Timestamp") {
    ...
    t.setNanos(1000)
    Decoder[WithTimestamp].decode(schema).apply(record) shouldBe WithTimestamp(timestamp(1538312231000L,1000))
  }
  ...
  test("decode timestamp-nanos to Timestamp") {
    ...
    Decoder[WithTimestamp].decode(schema).apply(record) shouldBe WithTimestamp(timestamp(1538312231000L, 1))
  }
  ...
  test("decode long to Instant") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 1538312231000L)
    Decoder[WithInstant].decode(schema).apply(record) shouldBe WithInstant(Instant.ofEpochMilli(1538312231000L))
  }
  ...
  test("decode timestamp-micros to Instant") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 1538312231000001L)
    Decoder[WithInstant].decode(schema).apply(record) shouldBe WithInstant(Instant.ofEpochMilli(1538312231000L).plusNanos(1000))
  }
  ...
  test("decode timestamp-nanos to Instant") {
    ...
    val record = new GenericData.Record(schema)
    record.put("z", 1538312231000000001L)
    Decoder[WithInstant].decode(schema).apply(record) shouldBe WithInstant(Instant.ofEpochMilli(1538312231000L).plusNanos(1))
  }
