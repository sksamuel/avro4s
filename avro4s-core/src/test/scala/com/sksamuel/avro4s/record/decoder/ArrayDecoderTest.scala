...
import com.sksamuel.avro4s.internal.{Decoder, Encoder, InternalRecord, SchemaEncoder}
...
class ArrayDecoderTest extends WordSpec with Matchers {
  ...
  "Decoder" should {
    ...
    "support array for a vector of primitives" in {
      val schema = SchemaEncoder[TestVectorBooleans].encode()
    ...
    "support array for an vector of records" in {
      ...
      val containerSchema = SchemaEncoder[TestVectorRecords].encode()
      val recordSchema = SchemaEncoder[Record].encode()
    ...
    "support array for an Array of primitives" in {
      val schema = SchemaEncoder[TestArrayBooleans].encode()
      val record = new GenericData.Record(schema)
      record.put("booleans", List(true, false, true).asJava)
    ...
    "support array for a List of primitives" in {
      ...
      val schema = SchemaEncoder[TestListBooleans].encode()
      val record = new GenericData.Record(schema)
      record.put("booleans", List(true, false, true).asJava)
    ...
    "support array for a List of records" in {
      ...
      val containerSchema = SchemaEncoder[TestListRecords].encode()
      val recordSchema = SchemaEncoder[Record].encode()
    ...
    "support array for a scala.collection.immutable.Seq of records" in {
      ...
      val containerSchema = SchemaEncoder[TestSeqRecords].encode()
      val recordSchema = SchemaEncoder[Record].encode()
    ...
    "support array for an Array of records" in {
      ...
      val containerSchema = SchemaEncoder[TestArrayRecords].encode()
      val recordSchema = SchemaEncoder[Record].encode()
    ...
    "support array for a Set of records" in {
      ...
      val containerSchema = SchemaEncoder[TestSetRecords].encode()
      val recordSchema = SchemaEncoder[Record].encode()
    ...
    "support array for a Set of strings" in {
      val schema = SchemaEncoder[TestSetString].encode()
      val record = new GenericData.Record(schema)
    ...
    "support array for a Set of doubles" in {
      val schema = SchemaEncoder[TestSetDoubles].encode()
      val record = new GenericData.Record(schema)
