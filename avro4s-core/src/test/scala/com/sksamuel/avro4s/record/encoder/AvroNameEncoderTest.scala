...
class AvroNameEncoderTest extends AnyFunSuite with Matchers {
  ...
  test("encoder should take into account @AvroName on a field") {
    ...
    val record = Encoder[AvroNameEncoderTest].encode(schema).apply(AvroNameEncoderTest("hello")).asInstanceOf[GenericRecord]
    record.get("bar") shouldBe new Utf8("hello")
  }

  // todo waiting for next release of magnolia
  // test("encoding sealed traits enums should take into account AvroName on enum object") {
  //   val schema = AvroSchema[Ship]
  //   val record = Encoder[Ship].encode(schema).apply(Ship(Atlantic)).asInstanceOf[GenericRecord]
  ...
  //    val record = Encoder[WaterproofBox].encode(schema).apply(data).asInstanceOf[GenericRecord]
  //    val abox = record.get("airtight_box").asInstanceOf[GenericRecord]
  //    val contents = abox.get("contents").asInstanceOf[GenericRecord]
  //    contents.get("length").toString.toDouble shouldBe 1.23D
  //  }

  // todo waiting for next release of magnolia
  //  test("support encoding and decoding sealed traits with empty namespaces") {
  //    val schema = AvroSchema[Spaceship]
  //    val spaceship = Spaceship(MiserableCosmos(true))
  //    val encoded = Encoder[Spaceship].encode(schema).apply(spaceship)
  //  val decoded = Decoder[Spaceship].decode(schema).apply(encoded)
  //    spaceship.cosmos shouldBe MiserableCosmos(true)
  //  }

  //  test("support decoding with empty namespaces") {
  //    val schema = AvroSchema[Spaceship]
  //    val spaceship = Spaceship(MiserableCosmos(true))
  //    val encoded = Encoder[Spaceship].encode(schema).apply(spaceship)
  //    val decoded = Decoder[Spaceship].decode(schema).apply(encoded)
