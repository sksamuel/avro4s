...
class SealedTraitEncoderTest extends AnyFunSuite with Matchers {
  ...
  test("support sealed traits of case classes") {
    ...
    val record = EncoderV2[Wrapper].encode(Wrapper(Wobble("foo"))).asInstanceOf[GenericRecord]
    ...
    wibble.getSchema shouldBe AvroSchemaV2[Wobble]
  }

  test("support trait subtypes fields with same name") {
    val schema = AvroSchemaV2[Trapper]
    val record = EncoderV2[Trapper].encode(Trapper(Tobble("foo", "bar"))).asInstanceOf[GenericRecord]
    ...
    tobble.getSchema shouldBe AvroSchemaV2[Tobble]
  }

  test("support trait subtypes fields with same name and same type") {
    val schema = AvroSchemaV2[Napper]
    val record = EncoderV2[Napper].encode(Napper(Nabble("foo", 44))).asInstanceOf[GenericRecord]
    val nobble = record.get("nibble").asInstanceOf[GenericRecord]
    nobble.get("str") shouldBe new Utf8("foo")
    nobble.get("age") shouldBe 44
    nobble.getSchema shouldBe AvroSchemaV2[Nabble]
  }

  test("support top level ADTs") {
    val schema = AvroSchemaV2[Nibble]
    val record = EncoderV2[Nibble].encode(Nabble("foo", 44)).asInstanceOf[GenericRecord]
    record.get("str") shouldBe new Utf8("foo")
    record.get("age") shouldBe 44
    record.getSchema shouldBe AvroSchemaV2[Nabble]
  }

  test("trait of case objects should be encoded as enum") {
    val schema = AvroSchemaV2[DibbleWrapper]
    EncoderV2[DibbleWrapper].encode(DibbleWrapper(Dobble)).asInstanceOf[GenericRecord].get("dibble") shouldBe new GenericData.EnumSymbol(schema, Dobble)
    EncoderV2[DibbleWrapper].encode(DibbleWrapper(Dabble)).asInstanceOf[GenericRecord].get("dibble") shouldBe new GenericData.EnumSymbol(schema, Dabble)
  }

  test("top level traits of case objects should be encoded as enum") {
    val schema = AvroSchemaV2[Dibble]
    EncoderV2[Dibble].encode(Dobble) shouldBe new GenericData.EnumSymbol(schema, Dobble)
    EncoderV2[Dibble].encode(Dabble) shouldBe new GenericData.EnumSymbol(schema, Dabble)
  }

  test("options of sealed traits should be encoded correctly") {
    val schema = AvroSchemaV2[MeasurableThing]
    val record = EncoderV2[MeasurableThing].encode(MeasurableThing(Some(WidthDimension(1.23)))).asInstanceOf[GenericRecord]
  ...
  test("classes nested in objects should be encoded correctly") {
    ...
    val record = EncoderV2[Outer].encode(Outer(InnerTwo(1.23))).asInstanceOf[GenericRecord]
  ...
  test("sealed trait classes with optional elements inside case classes should be encoded correctly") {
    ...
    case class InnerTwo(height: Double) extends Inner
    val schema = AvroSchemaV2[Outer]
    val record = EncoderV2[Outer].encode(Outer(InnerOne(1.23, None))).asInstanceOf[GenericRecord]
  }
}
