...
class SealedTraitEncoderTest extends FunSuite with Matchers {
  ...
  test("support sealed traits of case classes") {
    ...
    val record = Encoder[Wrapper].encode(Wrapper(Wobble("foo")), schema, DefaultFieldMapper).asInstanceOf[GenericRecord]
  ...
  test("support trait subtypes fields with same name") {
    ...
    val record = Encoder[Trapper].encode(Trapper(Tobble("foo", "bar")), schema, DefaultFieldMapper).asInstanceOf[GenericRecord]
  ...
  test("support trait subtypes fields with same name and same type") {
    ...
    val record = Encoder[Napper].encode(Napper(Nabble("foo", 44)), schema, DefaultFieldMapper).asInstanceOf[GenericRecord]
  ...
  test("support top level ADTs") {
    ...
    val record = Encoder[Nibble].encode(Nabble("foo", 44), schema, DefaultFieldMapper).asInstanceOf[GenericRecord]
  ...
  test("trait of case objects should be encoded as enum") {
    ...
    Encoder[DibbleWrapper].encode(DibbleWrapper(Dobble), schema, DefaultFieldMapper).asInstanceOf[GenericRecord].get("dibble") shouldBe new GenericData.EnumSymbol(schema, Dobble)
    Encoder[DibbleWrapper].encode(DibbleWrapper(Dabble), schema, DefaultFieldMapper).asInstanceOf[GenericRecord].get("dibble") shouldBe new GenericData.EnumSymbol(schema, Dabble)
  }

  test("top level traits of case objects should be encoded as enum") {
    val schema = AvroSchema[Dibble]
    Encoder[Dibble].encode(Dobble, schema, DefaultFieldMapper) shouldBe new GenericData.EnumSymbol(schema, Dobble)
    Encoder[Dibble].encode(Dabble, schema, DefaultFieldMapper) shouldBe new GenericData.EnumSymbol(schema, Dabble)
  }

  test("options of sealed traits should be encoded correctly") {
    val schema = AvroSchema[MeasurableThing]
    val record = Encoder[MeasurableThing].encode(MeasurableThing(Some(WidthDimension(1.23))), schema, DefaultFieldMapper).asInstanceOf[GenericRecord]
  ...
  test("classes nested in objects should be encoded correctly") {
    ...
    val record = Encoder[Outer].encode(Outer(InnerTwo(1.23)), schema, DefaultFieldMapper).asInstanceOf[GenericRecord]
  ...
  test("sealed trait classes with optional elements inside case classes should be encoded correctly") {
    ...
    case class InnerTwo(height: Double) extends Inner
    val schema = AvroSchema[Outer]
    val record = Encoder[Outer].encode(Outer(InnerOne(1.23, None)), schema, DefaultFieldMapper).asInstanceOf[GenericRecord]
  }
}
