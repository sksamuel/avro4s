...
import com.sksamuel.avro4s.{AvroSchema, AvroTransient, DefaultFieldMapper, Encoder, ImmutableRecord}
...
class AvroTransientEncoderTest extends AnyFunSuite with Matchers {
  ...
  test("encoder should skip @AvroTransient fields") {
    ...
    val record = Encoder[Foo].encode(Foo("a", "b", "c"), AvroSchema[Foo], DefaultFieldMapper).asInstanceOf[ImmutableRecord]
