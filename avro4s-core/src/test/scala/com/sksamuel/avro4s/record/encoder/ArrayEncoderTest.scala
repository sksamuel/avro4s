...
class ArrayEncoderTest extends AnyWordSpec with Matchers {
  ...
  "Encoder" should {
    "generate array for a vector of primitives" in {
      ...
      Encoder[Test].encode(schema).apply(Test(Vector(true, false, true))) shouldBe ImmutableRecord(schema, Vector(Vector(true, false, true).asJava))
    }
    "generate array for an vector of records" in {
      ...
      Encoder[Test].encode(schema).apply(Test(Vector(Record("abc", 12.34)))) shouldBe ImmutableRecord(schema, Vector(Vector(ImmutableRecord(rschema, Vector(new Utf8("abc"), java.lang.Double.valueOf(12.34)))).asJava))
    }
    "generate array for a scala.collection.immutable.Seq of primitives" in {
      case class Test(seq: Seq[String])
      val schema = AvroSchema[Test]
      Encoder[Test].encode(schema).apply(Test(Vector("a", "fgD"))) shouldBe ImmutableRecord(schema, Vector(Vector(new Utf8("a"), new Utf8("fgD")).asJava))
    }
    "generate array for an Array of primitives" in {
      case class Test(array: Array[Boolean])
      val schema = AvroSchema[Test]
      Encoder[Test].encode(schema).apply(Test(Array(true, false, true))) shouldBe ImmutableRecord(schema, Vector(Vector(true, false, true).asJava))
    }
    "generate array for a List of primitives" in {
      case class Test(list: List[String])
      val schema = AvroSchema[Test]
      Encoder[Test].encode(schema).apply(Test(List("we23", "54"))) shouldBe ImmutableRecord(schema, Vector(Vector(new Utf8("we23"), new Utf8("54")).asJava))
    }
    "generate array for a scala.collection.immutable.Seq of records" in {
      ...
      Encoder[Test].encode(schema).apply(Test(Seq(Nested("qwe"), Nested("dfsg")))) shouldBe ImmutableRecord(schema, Vector(Vector(ImmutableRecord(nschema, Vector(new Utf8("qwe"))), ImmutableRecord(nschema, Vector(new Utf8("dfsg")))).asJava))
    }
    "generate array for an Array of records" in {
      ...
      val schema = AvroSchema[Test]
      val nschema = AvroSchema[Nested]
      Encoder[Test].encode(schema).apply(Test(Array(Nested("qwe"), Nested("dfsg")))) shouldBe ImmutableRecord(schema, Vector(Vector(ImmutableRecord(nschema, Vector(new Utf8("qwe"))), ImmutableRecord(nschema, Vector(new Utf8("dfsg")))).asJava))
    }
    "generate array for a List of records" in {
      ...
      val schema = AvroSchema[Test]
      val nschema = AvroSchema[Nested]
      Encoder[Test].encode(schema).apply(Test(List(Nested("qwe"), Nested("dfsg")))) shouldBe ImmutableRecord(schema, Vector(Vector(ImmutableRecord(nschema, Vector(new Utf8("qwe"))), ImmutableRecord(nschema, Vector(new Utf8("dfsg")))).asJava))
    }
    "generate array for a Set of records" in {
      ...
      val nschema = AvroSchema[Nested]
      val schema = AvroSchema[Test]
      val record = Encoder[Test].encode(schema).apply(Test(Set(Nested("qwe"), Nested("dfsg")))).asInstanceOf[ImmutableRecord]
    ...
    "generate array for a Set of strings" in {
      ...
      val schema = AvroSchema[Test]
      Encoder[Test].encode(schema).apply(Test(Set("we23", "54"))) shouldBe ImmutableRecord(schema, Vector(Vector(new Utf8("we23"), new Utf8("54")).asJava))
    }
    "generate array for a Set of doubles" in {
      case class Test(set: Set[Double])
      val schema = AvroSchema[Test]
      Encoder[Test].encode(schema).apply(Test(Set(1.2, 34.5, 54.3))) shouldBe ImmutableRecord(schema, Vector(Vector(1.2, 34.5, 54.3).asJava))
    }
    ...
    "support top level Seq[Double]" in {
      ...
      Encoder[Array[Double]].encode(schema).apply(Array(1.2, 34.5, 54.3)) shouldBe new GenericData.Array[Double](schema, List(1.2, 34.5, 54.3).asJava)
    }
    "support top level List[Int]" in {
      val schema = AvroSchema[List[Int]]
      Encoder[List[Int]].encode(schema).apply(List(1, 4, 9)) shouldBe new GenericData.Array[Int](schema, List(1, 4, 9).asJava)
    }
    "support top level Vector[String]" in {
      val schema = AvroSchema[Vector[String]]
      Encoder[Vector[String]].encode(schema).apply(Vector("a", "z")) shouldBe new GenericData.Array[Utf8](schema, List(new Utf8("a"), new Utf8("z")).asJava)
    }
    "support top level Set[Boolean]" in {
      val schema = AvroSchema[Set[Boolean]]
      Encoder[Set[Boolean]].encode(schema).apply(Set(true, false, true)) shouldBe new GenericData.Array[Boolean](schema, Set(true, false).asJava)
    }
  }
}
