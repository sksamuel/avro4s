...
class AvroDocTest extends AnyWordSpec with Matchers {

  "@AvroDoc" should {
    "support doc annotation on class" in {
      @AvroDoc("hello; is it me you're looking for") case class Annotated(str: String)
      val expected = new org.apache.avro.Schema.Parser().parse(getClass.getResourceAsStream("/schemas/avrodoc/class.json"))
      val schema = AvroSchema[Annotated]
      schema.toString(true) shouldBe expected.toString(true)
    }
    "support doc annotation on field" in {
      case class Annotated(@AvroDoc("string doc") str: String, @AvroDoc("long doc") long: Long, int: Int)
      val expected = new org.apache.avro.Schema.Parser().parse(getClass.getResourceAsStream("/schemas/avrodoc/field.json"))
      val schema = AvroSchema[Annotated]
      schema.toString(true) shouldBe expected.toString(true)
    }
