...
class ArraySchemaTest extends AnyWordSpec with Matchers {
  ...
  "SchemaEncoder" should {
    "generate array type for a vector of primitives" in {
      ...
      val schema = AvroSchemaV2[VectorPrim]
    ...
    "generate array type for an vector of records" in {
      ...
      val schema = AvroSchemaV2[VectorRecord]
      schema.toString(true) shouldBe expected.toString(true)
    }
    ...
    "generate array type for a List of primitives" in {
      ...
      val schema = AvroSchemaV2[NestedListString]
      schema.toString(true) shouldBe expected.toString(true)
    }
    ...
    "generate array type for a Set of records" in {
      ...
      val schema = AvroSchemaV2[NestedSet]
      schema.toString(true) shouldBe expected.toString(true)
    }
    ...
    "generate array type for a Set of doubles" in {
      ...
      val schema = AvroSchemaV2[NestedSetDouble]
      schema.toString(true) shouldBe expected.toString(true)
    }
    "support Seq[Tuple2] issue #156" in {
      ...
      val schema = AvroSchemaV2[TupleTest2]
      schema.toString(true) shouldBe expected.toString(true)
    }
    "support Seq[Tuple3]" in {
      ...
      val schema = AvroSchemaV2[TupleTest3]
      schema.toString(true) shouldBe expected.toString(true)
    }
