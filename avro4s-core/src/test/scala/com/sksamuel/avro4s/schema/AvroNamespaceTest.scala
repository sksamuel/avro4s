...
//      case class AnnotatedNamespace(s: String, @AvroNamespace("com.yuval.internal") internal: InternalAnnotated)
//
//      val expected = new org.apache.avro.Schema.Parser().parse(getClass.getResourceAsStream("/schemas/namespace/field.json"))
//      val schema = AvroSchema[AnnotatedNamespace]
//      schema.toString(true) shouldBe expected.toString(true)
//    }
...
//    }
    ...
    "remove namespace when using empty string" in {
      ...
      val expected = new org.apache.avro.Schema.Parser().parse(getClass.getResourceAsStream("/schemas/namespace/empty_namespace.json"))
