...
class RecursiveSchemaTest extends AnyWordSpec with Matchers {
  ...
  "SchemaFor" should {
    ...
    "support recursive types with option" in {
      val expected = new org.apache.avro.Schema.Parser().parse(getClass.getResourceAsStream("/schemas/recursive/option.json"))
      val schema = AvroSchema[OptionTree]
      schema.toString(true) shouldBe expected.toString(true)
      //      AvroSchema[Testy] shouldBe expectedSchema("/schemas/recursive/option.json")
    }
...
case class OptionTree(value: String, left: Option[OptionTree])
