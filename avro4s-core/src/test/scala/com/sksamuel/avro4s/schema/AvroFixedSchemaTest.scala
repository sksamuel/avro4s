...
import com.sksamuel.avro4s.{AvroFixed, AvroSchemaV2}
import org.scalatest.matchers.should.Matchers
import org.scalatest.wordspec.AnyWordSpec

class AvroFixedSchemaTest extends AnyWordSpec with Matchers {

  "@AvroFixed" should {

    "generated fixed field schema when used on a field" in {
      val schema = AvroSchemaV2[FixedStringField]
    ...
    "generated fixed schema when an annotated value type is used as the type in a field" in {
      val schema = AvroSchemaV2[FooWithValue]
    ...
    "generate fixed schema for an annotated top level value type" in {
      val schema = AvroSchemaV2[FixedValueClass]
