...
case class SeqOfCaseClassWithMaps(ttt: HasMap3)
case class HasMap3(qqq: HasMap2)
case class HasMap2(ss: Map[String, String])
...
class AvroInputStreamTest extends WordSpec with Matchers with Timeouts {
  ...
  "AvroInputStream" should {
    ...
    //      in.close()
    //    }
    "read seqs of maps" in {
      //      val data = SeqOfCaseClassWithMaps(Seq(HasMap(Map("a" -> "b")), HasMap(Map("c" -> "d"))))
      //      val bytes = write(Seq(data))
      //      val in = AvroInputStream[SeqOfCaseClassWithMaps](bytes)
