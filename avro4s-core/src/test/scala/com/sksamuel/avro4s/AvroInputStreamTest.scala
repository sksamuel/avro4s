...
class AvroInputStreamTest extends WordSpec with Matchers with Timeouts {
  ...
  def write[T](ts: Seq[T])(implicit schema: Lazy[AvroSchema[T]], ser: Lazy[AvroWriter[T]]): Array[Byte] = {
  ...
  "AvroDeserializer" should {
    ...
    "read set of strings" in {
      ...
      val data = Seq(Test(Set("sammy", "hammy")))
      val bytes = write(data)
    ...
    "read set of case classes" in {
      ...
      val data = Seq(Test(Set(Nested("sammy", 45), Nested("hammy", 12.3d))))
      val bytes = write(data)
