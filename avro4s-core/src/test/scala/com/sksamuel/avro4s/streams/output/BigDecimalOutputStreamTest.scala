...
class BigDecimalOutputStreamTest extends OutputStreamTest {
  ...
  test("write seq of big decimal") {
    ...
    writeRead(Test(Seq(56.3, 179.2))) { record =>
      ...
      buffer1.get(bytes1)
      BigDecimal(BigInt(bytes1), 2) shouldBe BigDecimal(56.30)
      val buffer2 = array.get(1)
      val bytes2 = Array.ofDim[Byte](buffer2.remaining())
      buffer2.get(bytes2)
      BigDecimal(BigInt(bytes2), 2) shouldBe BigDecimal(179.20)
    }
  }

  test("write option of big decimal") {
    case class Test(z: Option[BigDecimal])
    writeRead(Test(Some(56.3))) { record =>
      val buffer = record.get("z").asInstanceOf[ByteBuffer]
      val bytes = Array.ofDim[Byte](buffer.remaining())
      buffer.get(bytes)
      BigDecimal(BigInt(bytes), 2) shouldBe BigDecimal(56.30)
    }
