...
import com.sksamuel.avro4s.{AvroInputStream2, AvroOutputStream2}
...
class BinaryStreamsTest extends WordSpec with Matchers {
  ...
  "Avro binary streams" should {
    "read and write" in {
      ...
      implicit val schema = SchemaEncoder[Composer]
      implicit val encoder = Encoder[Composer]

      val baos = new ByteArrayOutputStream()
      val output = AvroOutputStream2.binary[Composer](baos)
      output.write(ennio)
      output.write(hans)
      output.close()
