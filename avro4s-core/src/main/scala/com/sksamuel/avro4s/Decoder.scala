...
trait Decoder[T] extends Serializable {
  ...
  def withNamingStrategy(newNamingStrategy: NamingStrategy): Decoder[T] =
    Decoder((v, s, _) => decode(v, s), newNamingStrategy)
  ...
  def map[U](fn: T => U): Decoder[U] = Decoder[U] (
    (value: Any, schema: Schema, _: NamingStrategy) => fn(self.decode(value: Any, schema: Schema))
  )
}

object Decoder {

  def apply[T](dec: (Any, Schema, NamingStrategy) => T): Decoder[T] = apply(dec, DefaultNamingStrategy)

  def apply[T](dec: (Any, Schema, NamingStrategy) => T, naming: NamingStrategy): Decoder[T] =
    new Decoder[T] {

      override def withNamingStrategy(newNamingStrategy: NamingStrategy): Typeclass[T] =
        if (naming == newNamingStrategy) this
        else Decoder(dec, newNamingStrategy)

      override def decode(value: Any, schema: Schema): T = dec(value, schema, naming)
    }

  def apply[T](implicit decoder: Decoder[T], naming: NamingStrategy = DefaultNamingStrategy): Decoder[T] =
    decoder.withNamingStrategy(naming)
  ...
  implicit def optionDecoder[T](implicit decoder: Decoder[T]) = Decoder[Option[T]] {
    (value: Any, schema: Schema, naming: NamingStrategy) => if (value == null) None else {
      ...
      Option(decoder.withNamingStrategy(naming).decode(value, nonNullSchema))
    }
  }
  ...
  implicit def seqDecoder[T](implicit decoder: Decoder[T]): Decoder[Seq[T]] = Decoder[Seq[T]] {

    import scala.collection.JavaConverters._

    (value: Any, schema: Schema, naming: NamingStrategy) => value match {
      case array: Array[_] =>
        array.toSeq.map(decoder.withNamingStrategy(naming).decode(_, schema.getElementType))
      case list: java.util.Collection[_] =>
        list.asScala.map(decoder.withNamingStrategy(naming).decode(_, schema.getElementType)).toSeq
      case other => sys.error("Unsupported array " + other)
    }
  }

  implicit def mapDecoder[T](implicit valueDecoder: Decoder[T]): Decoder[Map[String, T]] = Decoder[Map[String, T]] {
    ...
    (value: Any, schema: Schema, naming: NamingStrategy) => value match {
      case map: java.util.Map[_, _] =>
        map.asScala.toMap.map { case (k, v) =>
          k.toString -> valueDecoder.withNamingStrategy(naming).decode(v, schema.getValueType)
        }
...
//    private val nameB = Namer(implicitly[WeakTypeTag[B]].tpe).fullName
  ...
  def combine[T](ctx: CaseClass[Typeclass, T]): Decoder[T] = {
    ...
    if (ctx.isValueClass) {
      Decoder[T] {
        (value: Any, schema: Schema, naming: NamingStrategy) => {
          val decoded = ctx.parameters.head.typeclass.withNamingStrategy(naming).decode(value, schema)
              ...
              val values = ctx.parameters.map { p =>
                ...
                if (extractor.transient || field == null) {
                  p.default match {
                    ...
                    case None => p.typeclass.withNamingStrategy(naming)
                      .decode(null, record.getSchema)
                  }
                } else {
                  val k = record.getSchema.getFields.indexOf(field)
                  val value = record.get(k)
                  p.typeclass.withNamingStrategy(naming)
                    .decode(value, schema.getFields.get(p.index).schema)
                }
              }
  ...
  def dispatch[T](ctx: SealedTrait[Typeclass, T]): Decoder[T] = Decoder[T] {
    (container: Any, schema: Schema, naming: NamingStrategy) => {
      schema.getType match {
        case Schema.Type.RECORD =>
          container match {
            case container: GenericContainer =>
              ...
              subtype.typeclass.withNamingStrategy(naming).decode(container, schema)
          ...
          container match {
            case container: GenericContainer =>
              ...
              val subtype = ctx.subtypes.find { subtype => Namer(subtype.typeName, subtype.annotations).fullName == container.getSchema.getFullName }
                .getOrElse(sys.error(s"Could not find subtype for ${container.getSchema.getFullName} in subtypes ${ctx.subtypes}"))
              subtype.typeclass.withNamingStrategy(naming).decode(container, subschema)
            case _ => sys.error(s"Unsupported type $container in sealed trait decoder")
          }
          ...
          container match {
            case enum: GenericEnumSymbol[_] =>
              ctx.subtypes.find { subtype => Namer(subtype).name == enum.getSchema.getFullName }
                ...
                .typeclass.withNamingStrategy(naming).decode(enum, enum.getSchema)
            case str: String =>
  ...
  implicit def tuple2Decoder[A, B](implicit
    ...
    (t: Any, schema: Schema, naming: NamingStrategy) => {
      val record = t.asInstanceOf[GenericRecord]
      (
        decoderA.withNamingStrategy(naming).decode(record.get("_1"), schema),
        decoderB.withNamingStrategy(naming).decode(record.get("_2"), schema)
      )
    }
  }
  ...
  implicit def tuple3Decoder[A, B, C, D, E](implicit
    ...
    (t: Any, schema: Schema, naming: NamingStrategy) => {
      ...
      (
        decoderA.withNamingStrategy(naming).decode(record.get("_1"), schema),
        decoderB.withNamingStrategy(naming).decode(record.get("_2"), schema),
        decoderC.withNamingStrategy(naming).decode(record.get("_3"), schema)
      )
    }
  }
  ...
  implicit def tuple4Decoder[A, B, C, D, E](implicit
    ...
    (t: Any, schema: Schema, naming: NamingStrategy) => {
      ...
      (
        ...
        decoderB.withNamingStrategy(naming).decode(record.get("_2"), schema),
        decoderC.withNamingStrategy(naming).decode(record.get("_3"), schema),
        decoderD.withNamingStrategy(naming).decode(record.get("_4"), schema)
      )
    }
  }
  ...
  implicit def tuple5Decoder[A, B, C, D, E](implicit
    ...
    (t: Any, schema: Schema, naming: NamingStrategy) => {
      ...
      (
        ...
        decoderC.withNamingStrategy(naming).decode(record.get("_3"), schema),
        decoderD.withNamingStrategy(naming).decode(record.get("_4"), schema),
        decoderE.withNamingStrategy(naming).decode(record.get("_5"), schema)
      )
    }
  }
