...
object ToRecord {
  ...
  def apply[T](schema: Schema)(implicit encoder: Encoder[T], naming: NamingStrategy = DefaultNamingStrategy): ToRecord[T] = new ToRecord[T] {
    override def to(t: T): Record = encoder.encode(t, schema, naming) match {
