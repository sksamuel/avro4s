...
trait SchemaFor[T] extends Serializable {
  ...
  def schema(fieldMapper: FieldMapper, context: SchemaFor.Context = SchemaFor.Context.empty): Schema
  ...
  def map[U](fn: Schema => Schema): SchemaFor[U] = new SchemaFor[U] {
    override def schema(fieldMapper: FieldMapper, context: SchemaFor.Context): Schema = fn(self.schema(fieldMapper, context))
  }
}
...
object SchemaFor {
  ...
  implicit val StringSchemaFor: SchemaFor[String] = (_, _) => SchemaBuilder.builder.stringType
  implicit val LongSchemaFor: SchemaFor[Long] = (_, _) => SchemaBuilder.builder.longType
  implicit val IntSchemaFor: SchemaFor[Int] = (_, _) => SchemaBuilder.builder.intType
  implicit val DoubleSchemaFor: SchemaFor[Double] = (_, _) => SchemaBuilder.builder.doubleType
  implicit val FloatSchemaFor: SchemaFor[Float] = (_, _) => SchemaBuilder.builder.floatType
  implicit val BooleanSchemaFor: SchemaFor[Boolean] = (_, _) => SchemaBuilder.builder.booleanType
  implicit val ByteArraySchemaFor: SchemaFor[Array[Byte]] = (_, _) => SchemaBuilder.builder.bytesType
  implicit val ByteSeqSchemaFor: SchemaFor[Seq[Byte]] = (_, _) => SchemaBuilder.builder.bytesType
  implicit val ByteListSchemaFor: SchemaFor[List[Byte]] = (_, _) => SchemaBuilder.builder.bytesType
  implicit val ByteVectorSchemaFor: SchemaFor[Vector[Byte]] = (_, _) => SchemaBuilder.builder.bytesType
  implicit val ByteBufferSchemaFor: SchemaFor[ByteBuffer] = (_, _) => SchemaBuilder.builder.bytesType
  implicit val ShortSchemaFor: SchemaFor[Short] = (_, _) => IntSchemaFor.schema(DefaultFieldMapper)
  implicit val ByteSchemaFor: SchemaFor[Byte] = (_, _) => IntSchemaFor.schema(DefaultFieldMapper)

  implicit object Utf8Schema extends SchemaFor[Utf8] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = Schema.create(Schema.Type.STRING)
  }

  implicit object UUIDSchemaFor extends SchemaFor[UUID] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = LogicalTypes.uuid().addToSchema(SchemaBuilder.builder.stringType)
  }

  implicit def mapSchemaFor[V](implicit schemaFor: SchemaFor[V]): SchemaFor[Map[String, V]] = {
    new SchemaFor[Map[String, V]] {
      override def schema(fieldMapper: FieldMapper, context: Context): Schema = SchemaBuilder.map().values(schemaFor.schema(fieldMapper, context))
    }
  }
  ...
  implicit def bigDecimalFor(implicit sp: ScalePrecision = ScalePrecision.default): SchemaFor[BigDecimal] = new SchemaFor[BigDecimal] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = LogicalTypes.decimal(sp.precision, sp.scale).addToSchema(SchemaBuilder.builder.bytesType)
  }
  ...
  implicit def eitherSchemaFor[A, B](implicit leftFor: SchemaFor[A], rightFor: SchemaFor[B]): SchemaFor[Either[A, B]] = {
    new SchemaFor[Either[A, B]] {
      override def schema(fieldMapper: FieldMapper, context: Context): Schema = SchemaHelper.createSafeUnion(leftFor.schema(fieldMapper, context), rightFor.schema(fieldMapper, context))
    }
  }

  implicit def optionSchemaFor[T](implicit schemaFor: SchemaFor[T]): SchemaFor[Option[T]] = new SchemaFor[Option[T]] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = {
      val elementSchema = schemaFor.schema(fieldMapper, context)
  ...
  implicit def seqSchemaFor[S](implicit schemaFor: SchemaFor[S]): SchemaFor[Seq[S]] = {
    new SchemaFor[Seq[S]] {
      override def schema(fieldMapper: FieldMapper, context: Context): Schema = {
        Schema.createArray(schemaFor.schema(fieldMapper, context))
      }
    }
  }
  ...
  implicit object TimestampSchemaFor extends SchemaFor[Timestamp] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = LogicalTypes.timestampMillis().addToSchema(SchemaBuilder.builder.longType)
  }

  implicit object LocalTimeSchemaFor extends SchemaFor[LocalTime] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = LogicalTypes.timeMicros().addToSchema(SchemaBuilder.builder.longType())
  }

  implicit object LocalDateSchemaFor extends SchemaFor[LocalDate] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = LogicalTypes.date().addToSchema(SchemaBuilder.builder.intType)
  }
  ...
  implicit object OffsetDateTimeSchemaFor extends SchemaFor[OffsetDateTime] {
    ...
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = OffsetDateTimeLogicalType.addToSchema(SchemaBuilder.builder().stringType())
  }
  ...
  def combine[T](ctx: CaseClass[Typeclass, T]): SchemaFor[T] = new SchemaFor[T] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = {
      ...
      context.getCachedSchema(name, namespace) match {
        case Some(recursiveSchema) => recursiveSchema
        case None =>
          ...
          if (ctx.isValueClass) {
            ...
            extractor.fixed match {
              case Some(size) =>
                ...
                builder.size(size)
              case None => param.typeclass.schema(fieldMapper, context)
            }
            ...
            val fields = ctx.parameters.flatMap { param =>
              ...
              if (annos.transient) None else {
                val s = param.typeclass.schema(fieldMapper, newContext)
  ...
  def dispatch[T: WeakTypeTag](ctx: SealedTrait[Typeclass, T]): SchemaFor[T] = new SchemaFor[T] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = {
      ...
      if (objs) {
        ...
        val schemas = sortedSubtypes.map(_.typeclass.schema(fieldMapper, context))
  ...
  implicit def scalaEnumSchemaFor[E <: scala.Enumeration#Value](implicit tag: TypeTag[E]): SchemaFor[E] = new SchemaFor[E] {
    ...
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = schema
  }
  ...
  implicit def coproductBaseSchema[S](implicit basefor: SchemaFor[S]): SchemaFor[S :+: CNil] = new SchemaFor[S :+: CNil] {
    ...
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = {
      val base = basefor.schema(fieldMapper, context)
  ...
  implicit def coproductSchema[S, T <: Coproduct](implicit basefor: SchemaFor[S], coproductFor: SchemaFor[T]): SchemaFor[S :+: T] = new SchemaFor[S :+: T] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema = {
      val base = basefor.schema(fieldMapper, context)
      val coproduct = coproductFor.schema(fieldMapper, context)
  ...
  implicit def tuple2SchemaFor[A, B](implicit a: SchemaFor[A], b: SchemaFor[B]): SchemaFor[(A, B)] = new SchemaFor[(A, B)] {
    override def schema(fieldMapper: FieldMapper, context: Context): Schema =
      SchemaBuilder.record("Tuple2").namespace("scala").doc(null)
        .fields()
        .name("_1").`type`(a.schema(fieldMapper, context)).noDefault()
        .name("_2").`type`(b.schema(fieldMapper, context)).noDefault()
        .endRecord()
  }
  ...
  implicit def tuple3SchemaFor[A, B, C](implicit
    ...
    override def schema(fieldMapper: FieldMapper, context: Context): Schema =
      SchemaBuilder.record("Tuple3").namespace("scala").doc(null)
        ...
        .name("_1").`type`(a.schema(fieldMapper, context)).noDefault()
        .name("_2").`type`(b.schema(fieldMapper, context)).noDefault()
        .name("_3").`type`(c.schema(fieldMapper, context)).noDefault()
        .endRecord()
  }
  ...
  implicit def tuple4SchemaFor[A, B, C, D](implicit
    ...
    override def schema(fieldMapper: FieldMapper, context: Context): Schema =
      SchemaBuilder.record("Tuple4").namespace("scala").doc(null)
        ...
        .name("_2").`type`(b.schema(fieldMapper, context)).noDefault()
        .name("_3").`type`(c.schema(fieldMapper, context)).noDefault()
        .name("_4").`type`(d.schema(fieldMapper, context)).noDefault()
        .endRecord()
  }
  ...
  implicit def tuple5SchemaFor[A, B, C, D, E](implicit
    ...
    override def schema(fieldMapper: FieldMapper, context: Context): Schema =
      SchemaBuilder.record("Tuple5").namespace("scala").doc(null)
        ...
        .name("_3").`type`(c.schema(fieldMapper, context)).noDefault()
        .name("_4").`type`(d.schema(fieldMapper, context)).noDefault()
        .name("_5").`type`(e.schema(fieldMapper, context)).noDefault()
        .endRecord()
  }
