...
/**
  ...
  */
abstract class SchemaFor[T: ClassTag] extends Serializable {
  self =>
  ...
  /**
    ...
    */
  def map[U: ClassTag](fn: Schema => Schema): SchemaFor[U] = new SchemaFor[U] {
...
object ScalaEnumSchemaFor extends EnumSchemaFor {
  ...
  def apply[E <: scala.Enumeration#Value : ClassTag](default: E)(implicit tag: TypeTag[E]): SchemaFor[E] = SchemaFor.scalaEnumSchemaFor.map[E](addDefault(default))
}
...
object SchemaFor {
  ...
  def apply[T: ClassTag](f: (FieldMapper, Map[Class[_], Schema]) => Schema): SchemaFor[T] = new SchemaFor[T] {
  ...
  implicit val StringSchemaFor: SchemaFor[String] = SchemaFor { (_, _) => SchemaBuilder.builder.stringType }
  implicit val LongSchemaFor: SchemaFor[Long] = SchemaFor { (_, _) => SchemaBuilder.builder.longType }
  implicit val IntSchemaFor: SchemaFor[Int] = SchemaFor { (_, _) => SchemaBuilder.builder.intType }
  implicit val DoubleSchemaFor: SchemaFor[Double] = SchemaFor { (_, _) => SchemaBuilder.builder.doubleType }
  implicit val FloatSchemaFor: SchemaFor[Float] = SchemaFor { (_, _) => SchemaBuilder.builder.floatType }
  implicit val BooleanSchemaFor: SchemaFor[Boolean] = SchemaFor { (_, _) =>SchemaBuilder.builder.booleanType }
  implicit val ByteArraySchemaFor: SchemaFor[Array[Byte]] = SchemaFor { (_, _) => SchemaBuilder.builder.bytesType }
  implicit val ByteSeqSchemaFor: SchemaFor[Seq[Byte]] = SchemaFor { (_, _) => SchemaBuilder.builder.bytesType }
  implicit val ByteListSchemaFor: SchemaFor[List[Byte]] = SchemaFor { (_, _) =>SchemaBuilder.builder.bytesType }
  implicit val ByteVectorSchemaFor: SchemaFor[Vector[Byte]] = SchemaFor { (_, _) => SchemaBuilder.builder.bytesType }
  implicit val ByteBufferSchemaFor: SchemaFor[ByteBuffer] = SchemaFor { (_, _) => SchemaBuilder.builder.bytesType }
  implicit val ShortSchemaFor: SchemaFor[Short] = SchemaFor { (_, _) => IntSchemaFor.schema(DefaultFieldMapper) }
  implicit val ByteSchemaFor: SchemaFor[Byte] = SchemaFor { (_, _) => IntSchemaFor.schema(DefaultFieldMapper) }

  implicit object Utf8Schema extends SchemaFor[Utf8] {
    override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema]): Schema = Schema.create(Schema.Type.STRING)
  }

  implicit object UUIDSchemaFor extends SchemaFor[UUID] {
    override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema]): Schema = LogicalTypes.uuid().addToSchema(SchemaBuilder.builder.stringType)
  }

  implicit def mapSchemaFor[V](implicit schemaFor: SchemaFor[V]): SchemaFor[Map[String, V]] = {
    new SchemaFor[Map[String, V]] {
      override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema] = Map()): Schema = SchemaBuilder.map().values(schemaFor.lookupOrCreate(fieldMapper, parents))
    }
  }
  ...
  implicit def bigDecimalFor(implicit sp: ScalePrecision = ScalePrecision.default): SchemaFor[BigDecimal] = new SchemaFor[BigDecimal] {
    override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema] = Map()): Schema = LogicalTypes.decimal(sp.precision, sp.scale).addToSchema(SchemaBuilder.builder.bytesType)
  }
  ...
  implicit def optionSchemaFor[T](implicit schemaFor: SchemaFor[T]): SchemaFor[Option[T]] = new SchemaFor[Option[T]] {
    override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema] = Map()): Schema = {
      val elementSchema = schemaFor.lookupOrCreate(fieldMapper, parents)
      val nullSchema = SchemaBuilder.builder().nullType()
      SchemaHelper.createSafeUnion(elementSchema, nullSchema)
    }
  }

  implicit def arraySchemaFor[S: ClassTag](implicit schemaFor: SchemaFor[S]): SchemaFor[Array[S]] = {
    new SchemaFor[Array[S]] {
      override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema] = Map()): Schema = Schema.createArray(schemaFor.lookupOrCreate(fieldMapper, parents))
    }
  }
  ...
  implicit def seqSchemaFor[S](implicit schemaFor: SchemaFor[S]): SchemaFor[Seq[S]] = {
    new SchemaFor[Seq[S]] {
      override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema] = Map()): Schema = {
        Schema.createArray(schemaFor.lookupOrCreate(fieldMapper, parents))
      }
    }
  }
  ...
  implicit object TimestampSchemaFor extends SchemaFor[Timestamp] {
    override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema] = Map()): Schema = LogicalTypes.timestampMillis().addToSchema(SchemaBuilder.builder.longType)
  }

  implicit object LocalTimeSchemaFor extends SchemaFor[LocalTime] {
    override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema] = Map()): Schema = LogicalTypes.timeMicros().addToSchema(SchemaBuilder.builder.longType())
  }

  implicit object LocalDateSchemaFor extends SchemaFor[LocalDate] {
    override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema] = Map()): Schema = LogicalTypes.date().addToSchema(SchemaBuilder.builder.intType)
  }
  ...
  implicit object OffsetDateTimeSchemaFor extends SchemaFor[OffsetDateTime] {
    ...
    override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema] = Map()): Schema = OffsetDateTimeLogicalType.addToSchema(SchemaBuilder.builder().stringType())
  }
  ...
  def combine[T: ClassTag](ctx: CaseClass[Typeclass, T]): SchemaFor[T] = new SchemaFor[T] {
  ...
  def dispatch[T: WeakTypeTag: ClassTag](ctx: SealedTrait[Typeclass, T]): SchemaFor[T] = new SchemaFor[T] {
    override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema]): Schema = {
  ...
  implicit def scalaEnumSchemaFor[E <: scala.Enumeration#Value: ClassTag](implicit tag: TypeTag[E]): SchemaFor[E] = new SchemaFor[E] {
    ...
    override def schema(fieldMapper: FieldMapper, parents: Map[Class[_], Schema]): Schema = schema
  }
